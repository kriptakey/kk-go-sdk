...
func main() {
	...
	ciphertext, err := kk.SealForTransit([]byte(data), DEMO_PUBLIC_KEY_OR_CERT)
	if err != nil {
		log.Fatal(err)
	}

	plaintext, err := connection.UnsealDataFromTransit(1, session.SessionToken, DEMO_WRAPPING_KEY_ID, DEMO_WRAPPED_KEY, ciphertext)
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println("- Plaintext: ", protojson.Format(plaintext))
